...
float getRate(string vehicleType, int & hourDifference, int & estimatedHour,int & estimatedMinute,int cutoff, 
				int est_Time, float & firstRate, float & secondRate);

//output
void print(string vehicleType, int hourIn, int minuteIn, int hourOut, int minuteOut, int & estimatedHour, 
...
totalRate = getRate(vehicleType, hourDifference, estimatedHour, estimatedMinute,cutoff, est_Time, firstRate, secondRate);
if ((select='C') || (select='V') || (select='S'))
{
print(vehicleType, hourIn, minuteIn, hourOut, minuteOut, estimatedHour, 
...
cout <<	"Please enter your time of departure, beginning with the hour value (06-24) and then minutes (00-59)" << endl;
...
float getRate(string vehicleType, int & hourDifference, int & estimatedHour,
				 int & estimatedMinute, int cutoff, int est_Time, float & firstRate, float & secondRate)
{
...
firstRate=1.00;
secondRate=2.50;
cutoff = 2; //To represent that after 2 hours, the function will use the second rate to charge the user

if (hourDifference <=2)
	return firstRate*estimatedHour;	
else
	{
	est_Time = estimatedHour-cutoff;
...
firstRate=2.00;
secondRate=3.50;
cutoff = 1; //To represent that after 3 hours, the function will use the second rate to charge the user
if (hourDifference == 1)
	{cout << "\n" << firstRate << endl;
	return firstRate;	
	}
	...
	{
	float totalRate = (secondRate*(estimatedHour-1));
	return totalRate;
	}
